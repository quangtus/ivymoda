...
class PromotionModel extends Model {
    ...
    public function getActivePromotions($limit = 5) {
        ...
        // Chỉ lấy các promotion có image_url (banner) và đang active
        // ORDER BY: priority DESC (cao nhất trước), sau đó start_date DESC (mới nhất trước)
        $query = "SELECT * FROM {$this->table}
                  ...
                  LIMIT $limit";
        $result = parent::getAll($query);
        
        // Debug: Log để kiểm tra thứ tự
        if (defined('ENVIRONMENT') && ENVIRONMENT === 'development') {
            error_log("Promotions order: " . json_encode(array_map(function($p) {
                return ['id' => $p->promotion_id, 'priority' => $p->priority, 'title' => $p->title];
            }, $result)));
        }
